        ...
        <div>
          ...
          <div class="relative">
            <input type="text" name="sku" value="{{ old('sku') }}" 
                   class="w-full bg-slate-800/50 border border-slate-600 rounded-xl px-4 py-3 focus:border-purple-500 focus:outline-none placeholder-slate-400" 
                   placeholder="SKU akan dihasilkan otomatis" id="sku-input" readonly>
            <input type="hidden" name="auto_generated" value="true" id="auto-generated">
            <div class="absolute right-3 top-3 flex items-center gap-2">
              <span id="sku-badge" class="text-xs px-2 py-1 rounded-full bg-purple-500/20 text-purple-400">
                <i class="fas fa-magic mr-1"></i>Otomatis
              </span>
            </div>
          </div>
          <p class="text-xs text-slate-400 mt-1">SKU akan dihasilkan otomatis dari 3 huruf pertama nama produk</p>
...
function generateSKU() {
  ...
  const autoGeneratedInput = document.getElementById('auto-generated');
  ...
  const words = nameInput.value.trim().split(/\s+/).slice(0, 3); // Take max 3 words
  const parts = words.map(word => word.substring(0, 3).toUpperCase());
  const prefix = parts.join('-');
  
  // Generate the next available number
  const nextNumber = String(skuCounter).padStart(3, '0');
  const newSKU = `${prefix}-${nextNumber}`;
  
  // Set the generated SKU
  skuInput.value = newSKU;
  autoGeneratedInput.value = 'true';
  isAutoGenerated = true;
  
  // Update UI
  skuInput.classList.add('sku-auto-generated');
  skuInput.classList.remove('sku-manual');
  skuBadge.innerHTML = '<i class="fas fa-magic mr-1"></i>Otomatis';
  skuBadge.className = 'text-xs px-2 py-1 rounded-full bg-purple-500/20 text-purple-400';
  
  // Increment counter for next SKU
  skuCounter++;
}
...
document.querySelector('input[name="name"]').addEventListener('input', function() {
  if (this.value.trim().length >= 3) {
    ...
    // Clear SKU if name is too short
    document.getElementById('sku-input').value = '';
    document.getElementById('auto-generated').value = 'false';
    isAutoGenerated = false;
  }
});

// Allow manual SKU editing
document.getElementById('sku-input').addEventListener('click', function() {
  if (isAutoGenerated) {
    // Switch to manual mode
    this.removeAttribute('readonly');
    this.classList.remove('sku-auto-generated');
    this.classList.add('sku-manual');
    
    const skuBadge = document.getElementById('sku-badge');
    skuBadge.innerHTML = '<i class="fas fa-edit mr-1"></i>Manual';
    skuBadge.className = 'text-xs px-2 py-1 rounded-full bg-green-500/20 text-green-400';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Initialize SKU counter from server
  skuCounter = 1;
  
  // Generate initial SKU if name is already filled
  const nameInput = document.querySelector('input[name="name"]');
