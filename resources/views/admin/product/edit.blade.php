...
<div class="glass-effect rounded-2xl p-6 mb-6">
  <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" id="product-form">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <!-- Left Column -->
      <div class="space-y-6">
        <!-- Product Image (Single like create form) -->
        <div>
          ...
          <div id="image-upload-area" class="product-image-preview hidden" onclick="document.getElementById('image-input').click()">
    ...
    <!-- Ukuran Sepatu (Aligned with create form) -->
    <div class="mt-6">
      ...
      <div id="sizes-container" class="space-y-3">
        ...
        <div class="size-input-row flex items-center gap-2" data-size-id="{{ $size->id }}">
          ...
          <input type="text" name="sizes[{{ $index }}][size]" value="{{ old('sizes.'.$index.'.size', $size->size) }}" placeholder="Ukuran (misal: 38, 39, S, M, L)" 
                 class="flex-1 bg-slate-800/50 border border-slate-600 rounded-xl px-4 py-3 focus:border-purple-500 focus:outline-none"
                 required>
          <input type="number" name="sizes[{{ $index }}][stock]" value="{{ old('sizes.'.$index.'.stock', $size->stock) }}" placeholder="Stok" min="0"
      ...
      <!-- Hidden input container for removed sizes -->
      <div id="removed-sizes-container"></div>
    </div>
...
// Single image preview functionality (aligned with create form)
...
// Size management functions (aligned with create form)
...
function removeSize(button) {
  ...
  // If this is an existing size (has data-size-id), mark it for deletion
  const sizeId = row.dataset.sizeId;
  if (sizeId) {
    removedSizes.push(sizeId);
    updateRemovedSizesInput();
  }
...
function updateRemovedSizesInput() {
  const container = document.getElementById('removed-sizes-container');
  container.innerHTML = removedSizes.map(id => 
    `<input type="hidden" name="removed_sizes[]" value="${id}">`
  ).join('');
}

// Format IDR currency input (aligned with create form)
...
// Price input formatting (aligned with create form)
...
// Form validation and submission (aligned with create form)
...
// Drag and drop functionality for single image (aligned with create form)
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Initialize drag and drop only if upload area exists and is visible
  if (imageUploadArea && !imageUploadArea.classList.contains('hidden')) {
    // Drag and drop is already set up above
  }
});
